...
public class GameControlbckup : MonoBehaviour {
	...
	void Update (){
		...
		if (jump == true) { 
		...
		} else if (currentPlayer.GetFreeze() == false) { //Jos pelaaja ei ole game over tai pause screenissä niin voi liikkua

			if (Input.GetKeyUp (KeyCode.W)) { //Jos W nostetaan pohjasta, jumpCap = true -> Pelaaja ei voi leijua spämmäämällä W:tä
				//				jumpCap = true;

			} else if (Input.GetKeyDown (KeyCode.W) && Input.GetKey (KeyCode.A) && jumpCap == false) { //Viisto hyppy vasemmalle
			...
			} else if (Input.GetKeyDown (KeyCode.W) && Input.GetKey (KeyCode.D) && jumpCap == false) { //Viisto hyppy oikealle
			...
			} else if (Input.GetKeyDown (KeyCode.W) && jumpCap == false) { //kun W painetaan pohjassa ja jumpCap == false pelaaja hyppää 
				jump = true;
	...
	void Jump() { //Hyppy
		...
		currentPlayerObject.transform.Translate (0, 5, 0); //Hypyn "räjähtävyys"
		jumpProgress++;
